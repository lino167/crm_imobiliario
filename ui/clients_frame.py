...
class ClientsFrame(ctk.CTkFrame):
    ...
    def on_item_select(self, event):
        ...
        if client_data:
            ...
            self.combo_tipo_interesse.set(data[9] or ""); self.entry_preco_min.insert(0, str(data[10] or "")); self.entry_preco_max.insert(0, str(data[11] or "")); self.entry_quartos_min.insert(0, str(data[12] or "")); self.entry_banheiros_min.insert(0, str(data[13] or "")); self.entry_vagas_min.insert(0, str(data[14] or "")); self.entry_bairros_interesse.insert(0, data[15] or ""); self.combo_finalidade.set(data[16] or "")
    def add_client(self):
        nome = self.entry_nome.get().strip()
        if not nome: messagebox.showerror("Erro de Validação", "O campo 'Nome Completo' é obrigatório."); return
        data_cadastro = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        try:
            preco_min = float(self.entry_preco_min.get()) if self.entry_preco_min.get() else None; preco_max = float(self.entry_preco_max.get()) if self.entry_preco_max.get() else None; quartos_min = int(self.entry_quartos_min.get()) if self.entry_quartos_min.get() else None; banheiros_min = int(self.entry_banheiros_min.get()) if self.entry_banheiros_min.get() else None; vagas_min = int(self.entry_vagas_min.get()) if self.entry_vagas_min.get() else None
        ...
        query = """INSERT INTO clientes (data_cadastro, nome_completo, status, telefone, email, proximo_contato, tipo_imovel_interesse, preco_min_interesse, preco_max_interesse, quartos_min_interesse, banheiros_min_interesse, vagas_min_interesse, bairros_interesse, finalidade) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""
        params = (data_cadastro, nome, self.combo_status.get(), self.entry_telefone.get(), self.entry_email.get(), self.entry_prox_contato.get(), self.combo_tipo_interesse.get(), preco_min, preco_max, quartos_min, banheiros_min, vagas_min, self.entry_bairros_interesse.get(), self.combo_finalidade.get())
        try:
    ...
    def update_client(self):
        ...
        query = """UPDATE clientes SET nome_completo=?, status=?, telefone=?, email=?, proximo_contato=?, tipo_imovel_interesse=?, preco_min_interesse=?, preco_max_interesse=?, quartos_min_interesse=?, banheiros_min_interesse=?, vagas_min_interesse=?, bairros_interesse=?, finalidade=? WHERE id=?"""
        params = (self.entry_nome.get(), self.combo_status.get(), self.entry_telefone.get(), self.entry_email.get(), self.entry_prox_contato.get(), self.combo_tipo_interesse.get(), preco_min, preco_max, quartos_min, banheiros_min, vagas_min, self.entry_bairros_interesse.get(), self.combo_finalidade.get(), client_id)
        try:
    ...
    def clear_fields(self, clear_selection=True):
        self.entry_nome.delete(0, 'end'); self.entry_telefone.delete(0, 'end'); self.entry_email.delete(0, 'end'); self.combo_status.set(""); self.entry_prox_contato.delete(0, 'end'); self.combo_tipo_interesse.set(""); self.entry_preco_min.delete(0, 'end'); self.entry_preco_max.delete(0, 'end'); self.entry_quartos_min.delete(0, 'end'); self.entry_banheiros_min.delete(0, 'end'); self.entry_vagas_min.delete(0, 'end'); self.entry_bairros_interesse.delete(0, 'end'); self.combo_finalidade.set("")
    ...
    # --- MÉTODO DE MATCHMAKING INTELIGENTE ---
    def find_matching_properties(self):
        ...
        # Perfil de busca do cliente
        p_tipo, p_preco_min, p_preco_max, p_quartos, p_banheiros, p_vagas = profile[0]

        # 1. Busca Ampla: Pega todos os imóveis que se encaixam na faixa de preço
        query = "SELECT codigo_ref, tipo, bairro, quartos, banheiros, vagas, preco_venda FROM imoveis WHERE status = 'Disponível'"
        params = []
        if p_preco_min is not None:
            query += " AND preco_venda >= ?"
            params.append(p_preco_min)
        if p_preco_max is not None:
            query += " AND preco_venda <= ?"
            params.append(p_preco_max)
        ...
        for prop in candidate_properties:
            ...
            # Comparando tipo
            if p_tipo and prop[1] == p_tipo:
                score += 50
            # Comparando quartos (imóvel pode ter mais quartos que o mínimo)
            if p_quartos and prop[3] is not None and prop[3] >= p_quartos:
                score += 25
            # Comparando banheiros
            if p_banheiros and prop[4] is not None and prop[4] >= p_banheiros:
                score += 15
            # Comparando vagas
            if p_vagas and prop[5] is not None and prop[5] >= p_vagas:
                score += 10
            
            # Formata o resultado para exibição, adicionando a pontuação
            # (Código, Tipo, Bairro, Quartos, Banheiros, Vagas, Preço, Pontuação)
            formatted_price = f"{prop[6]:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".") if prop[6] is not None else "0,00"
            scored_results.append(list(prop[:6]) + [formatted_price] + [f"{score}%"])

        # Ordena os resultados pela pontuação, da maior para a menor
        scored_results.sort(key=lambda x: int(x[7].strip('%')), reverse=True)

        # 3. Exibe os resultados
        columns = {
            "Código Ref.": 100, "Tipo": 100, "Bairro": 150, "Quartos": 60,
            "Banheiros": 70, "Vagas": 60, "Preço (R$)": 110, "Compatibilidade": 100
        }
        
        MatchResultsWindow(master=self.app, title="Resultado do Matchmaking Inteligente", results=scored_results, columns=columns)
